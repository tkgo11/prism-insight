...
# ChatGPT 구독으로 실행
PRISM_OPENAI_AUTH_MODE=chatgpt_oauth python stock_analysis_orchestrator.py --mode morning --language ko
```
...
이제 권장되는 빠른 시작 경로는 CUI 온보딩 위자드입니다. OpenAI API 키 또는 ChatGPT OAuth를 선택하고, 출력 언어를 정한 뒤, 명령줄에 시크릿을 노출하지 않고 안전한 첫 실행 명령을 생성할 수 있습니다.
...
로컬 온보딩은 `python onboard.py`를 우선 권장합니다. `quickstart.sh`는 셸 중심 흐름이 필요할 때 계속 사용할 수 있습니다.

위자드는 Apple(AAPL) AI 분석 리포트 같은 안전한 첫 실행 명령으로 마무리할 수 있습니다. 온보딩 후에는 다른 종목도 분석해보세요:
```bash
...
# 3. 분석 실행
docker exec -it prism-quickstart python3 demo.py NVDA --language ko
```
